...
const parseSort = (model, sortRaw) => {
    const defaultSort = { createdAt: -1 };
    if (!sortRaw || typeof sortRaw !== "string")
        return defaultSort;
    const [field, order] = sortRaw.split("_");
    if (!field || !model.schema.path(field))
        return defaultSort;
    return { [field]: order === "asc" ? 1 : -1 };
};
const castValueFromSchemaType = (schemaTypeName, rawValue) => {
...
const buildQuery = async (model, queryParams, searchFields, omitFields) => {
    ...
    const mongoQuery = {};
    const searchValue = typeof queryParams.search === "string" ? queryParams.search.trim() : "";
    if (searchValue) {
        const validSearchFields = searchFields.filter((field) => Boolean(model.schema.path(field)));
    ...
    Object.entries(queryParams).forEach(([key, rawValue]) => {
        ...
        if (!schemaPath)
            return;
        const value = castValueFromSchemaType(schemaPath.instance, rawValue);
        if (value === null)
            return;
