  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center gap-2 py-16 text-gray-400">
        ...
        <span className="text-sm">Loading course enquiry…</span>
      </div>
    );
  }

  if (!enquiry) {
    return <div className="p-8 text-center text-white/50">Course enquiry not found.</div>;
  }
      ...
      <div className="rounded-xl border border-white/10 bg-white/5 p-6 shadow-lg backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3">
          ...
          {(["New", "Contacted", "Interested", "Converted", "Closed"] as const).map((s) => (
            <Button key={s} size="sm" variant={enquiry.status === s ? "default" : "outline"} onClick={() => void updateStatus(s)} className="text-xs capitalize">
              {s}
            </Button>
          ))}
        </div>
